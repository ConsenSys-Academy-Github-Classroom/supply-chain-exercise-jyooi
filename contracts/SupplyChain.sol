...
contract SupplyChain {
    ...
    modifier checkItemState(uint256 _sku, State state) {
        require(items[_sku].state == state, "This item is for sale.");
        _;
    }
    ...
    modifier isSold(uint256 _sku) {
        require(items[_sku].state == State.Sold, "This item is not sold.");
        _;
    }
    ...
    constructor() public {
        ...
        owner = msg.sender;
        // 2. Initialize the sku count to 0. Question, is this necessary? // no as we can initialize it to 0 on the top 
